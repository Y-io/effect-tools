...
`@pkg/http-api-client/react` 将 Effect HttpApiClient endpoint 接入 TanStack React Query，同时保留 TanStack Query 的缓存、retry 和状态语义。
...
import { makeEffectReactRuntime } from "@pkg/http-api-client/react"
