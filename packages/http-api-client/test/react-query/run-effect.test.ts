...
import { QueryDefect } from "../../src/react-query/index"
import { runEffect } from "../../src/react-query/model"
...
test("runEffect 将 defect 包装为 QueryDefect", async () => {
  const defect = new Error("unexpected defect")

  await expect(runEffect(Runtime.defaultRuntime, Effect.die(defect))).rejects.toEqual(
    new QueryDefect({ cause: defect }),
  )
})

test("runEffect 将组合 Cause 包装为 QueryDefect", async () => {
  const effect = Effect.failCause(
    Cause.parallel(Cause.fail("left failure"), Cause.fail("right failure")),
  )

  await expect(runEffect(Runtime.defaultRuntime, effect)).rejects.toBeInstanceOf(QueryDefect)
})

test("runEffect 将 AbortSignal 中断包装为 QueryDefect", async () => {
  ...
  await expect(result).rejects.toBeInstanceOf(QueryDefect)
})
